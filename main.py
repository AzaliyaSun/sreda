...
def f(x):
    return [np.sin(x[0]+0.5) - x[1] - 1, x[0] + np.cos(x[1] - 2)]
...
def find_root_sys():
    ...
    res = [np.sin(x+0.5) - 1, 2 + np.arccos(-x)]
    f1 = plt.figure()
...
if __name__ == "__main__":
    ...
    print('\nЗадание 2.1')
    find_root(lambda x:  3 ** x + x*2 - 2)
    find_root_sys()

    print('\nЗадание 2.2.1')
    X = np.array([20, 22, 24, 26, 28])
    Y = np.array([40, 26.8, 18.0, 12.0, 8.1])
    least_sq(X, Y)

    print('\nЗадание 2.2.2')
    T = np.array([0, 0.41504, 0.83008, 1.24512, 1.66016, 2.0752])
    N = np.array([1000963, 943339, 892123, 840864, 794948, 749228])
